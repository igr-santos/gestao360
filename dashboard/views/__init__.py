...
from dashboard.templatetags.dashboard import display_numbers
...
def stakeholders(request):
    ...
    if stakeholder_id:
        ...
        splitlines = SplitLine.objects.filter(owner=stakeholder).order_by(
            "split__song__title"
        )
...
"""
    ...
    rows = list(
        map(
            lambda row: [
                row[0],
                display_numbers(row[1]),
                display_numbers(row[2]),
                display_numbers(row[3]),
                display_numbers(row[4]),
            ],
            rows,
        )
    )

    filename = f"[Demonstrativo] {report.title} - {stakeholder.full_name}"

    pdf = draw(
            stakeholder_name=stakeholder.full_name,
            title=f"Demonstrativo: {report.title}",
            header=[
                "TÍTULO",
                "QUANTIDADE",
                "RENDIMENTO (R$)",
            ...
            footer=["", "", "", "TOTAL", display_numbers(amount)],
            rows=rows,
        )
    ...
    # return HttpResponse(
    #     draw(
    #         stakeholder_name=f"{stakeholder.full_name}",
    #         title=f"Demonstrativo: {report.title}",
    #         header=[
    #             "TÍTULO",
    #             "QUANTIDADE",
